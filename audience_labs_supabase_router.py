...
REGION_ZIPS = {
    "riverside": ["92501", "92503", "92504", "92505", "92506", "92507", "92508", "92518"],
    "temecula": ["92589", "92590", "92591", "92592", "92593"],
    "murrieta": ["92562", "92563", "92564"],
    "menifee_sun_city": ["92584", "92585", "92586", "92587"],
    "lake_elsinore": ["92530", "92531", "92532"],
    "wildomar": ["92595"],
    "winchester_french_valley": ["92596"],
    "perris": ["92570", "92571", "92572", "92599"],
    "moreno_valley": ["92551", "92552", "92553", "92554", "92555", "92556", "92557"],
...
REGION_LABELS = {
    ...
    "temecula": "Temecula",
    "murrieta": "Murrieta",
    "menifee_sun_city": "Menifee / Sun City",
    "lake_elsinore": "Lake Elsinore",
    "wildomar": "Wildomar",
    "winchester_french_valley": "Winchester / French Valley",
    "perris": "Perris",
...
def process_lead(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "NET_WORTH": first_present(row, "NET_WORTH", "NETWORTH", "ESTIMATED_NET_WORTH", "EST_NET_WORTH"),
        "INCOME_RANGE": first_present(row, "INCOME_RANGE", "HOUSEHOLD_INCOME", "ESTIMATED_INCOME", "INCOME"),
        "CREDIT_RATING": first_present(row, "CREDIT_RATING", "CREDIT_SCORE_RANGE", "ESTIMATED_CREDIT_RATING", "CREDIT_SCORE"),
        "HOMEOWNER_STATUS": first_present(row, "HOMEOWNER_STATUS", "HOMEOWNER", "HOME_OWNER", "HOME_OWNERSHIP", "OWN_RENT", "RENTER_OWNER"),
        ...
        "PHONE_MATCH_SCORE": phone_candidate.match_score if phone_candidate else None,
...
def fetch_audience_rows_for_id(audience_id: str) -> list[dict[str, Any]]:
    ...
    while True:
        ...
        if not data:
            print(f"Finished fetching list {audience_id} at page {page - 1}.")
            break
        ...
        if last_batch_size is None:
            ...
            if batch_size and batch_size < PAGE_SIZE:
                print(
                    f"Audience Labs list {audience_id} is effectively returning {batch_size} rows per page "
                    f"even though page_size={PAGE_SIZE} was requested."
                )

        rows.extend(data)
        if page % 10 == 0:
            print(f"Downloaded {len(rows)} raw rows from list {audience_id}...")
